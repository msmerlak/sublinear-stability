...
function σ_crit(p;
    ...
    if !haskey(p, :dist_r)
        for i in 1:Iter
            ...
            e1_new = first(quadgk(x -> x*P_n(x, e1_n[i], e2_n[i], p) , n_min, n_max, rtol=tol))
            e2_new = first(quadgk(x -> x*x*P_n(x, e1_n[i], e2_n[i], p) , n_min, n_max, rtol=tol))
            σc_new = ((first(quadgk(x -> P_n(x, e1_n[i], e2_n[i], p)/((1-p[:k])*x^(p[:k]-2)-p[:μ]/p[:S])^2 , n_min, n_s - ϵ, rtol=tol))+
            first(quadgk(x -> P_n(x, e1_n[i], e2_n[i], p)/((1-p[:k])*x^(p[:k]-2)-p[:μ]/p[:S])^2 , n_s + ϵ, n_max, rtol=tol))-
